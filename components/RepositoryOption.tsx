...
import { classNames, formatCount } from '../lib/utils'
import {
  CpuChipIcon,
  StarIcon,
...
export function RepositoryOption(props: RepositoryOptionProps) {
  const { item } = props;
  return (
    <Combobox.Option
      value={item.html_url}
        ...
        <>
          <header className="flex items-center">
            <CpuChipIcon
              ...
              aria-hidden="true"
            />
            <span className="ml-1 font-bold flex-auto truncate">{item.full_name}</span>
          </header>
          ...
          <footer className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="flex items-center space-x-1">
                ...
                <span className="font-medium max-w-sm">{item.owner.login}</span>
              </span>
              {/* Language */}
              <span className="flex items-center space-x-1">
                <span className="block w-1.5 h-1.5 rounded-full bg-indigo-400" />
                <span className="font-medium">{item.language || 'Unknown'}</span>
              </span>
            </div>
            <div className="flex items-center space-x-2.5">
              ...
              <span className="flex items-center space-x-0.5">
                ...
                <span>{formatCount(item.stargazers_count)} stars</span>
              </span>
              {/* Issues */}
              <span className="flex items-center space-x-0.5">
                <ViewfinderCircleIcon className="w-4 h-4" />
                <span>{formatCount(item.open_issues_count)} issues</span>
              </span>
              {/* Forks */}
              <span className="flex items-center space-x-0.5">
                <svg
                ...
                <span>{formatCount(item.forks_count)} forks</span>
              </span>
            </div>
          </footer>
        </>
